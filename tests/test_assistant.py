...
from metro_agent.assistant.orchestrator import AssistantService, provider_from_environment
...
class AssistantTests(unittest.TestCase):
    ...
    def test_partial_failure_triggers_one_novel_replan(self) -> None:
    ...
    def test_clarification_stops_before_tools_and_persists_the_question(self) -> None:
            ...
            def generate_structured(self, prompt, schema, *, context):
                if schema is IntentEnvelope:
                    return IntentEnvelope(
                        task_type="query",
                        user_goal=context["question"],
                        ambiguities=["请明确要查询的客流指标。"],
        ...
        session = assistant.create_session()["session_id"]
        run = assistant.message(session, AssistantMessageRequest(message="帮我查一下"))
        self.assertEqual(run["status"], "needs_clarification")
        self.assertEqual(run["tool_results"], [])
        self.assertEqual(run["response"]["follow_up_questions"], ["请明确要查询的客流指标。"])
