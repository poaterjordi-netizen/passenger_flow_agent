# Public external-test real-data shadow demo
...
`production-shadow` runtime. The shared HTTP Basic Auth prompt was removed because embedded and
mobile browsers do not reliably support it. The URL is therefore suitable only for a bounded,
temporary external test and must not be described as a production service.
...
The live preview build is isolated under the external runtime data directory instead of
`clients/web/dist`. Routine developer builds therefore cannot replace the running
`/real-shadow/` asset paths with root-relative `/assets/` paths.

## Public gateway rules

The ECS Nginx configuration must keep these controls together:

- public access only to the explicitly enumerated `/real-shadow/` Web, health, bounded API, and
...
1. `/real-shadow/` opens without a shared browser-password prompt and returns `200`;
...
8. restarting the bridge restores the same fixed URL.
...
enterprise model platform, add a real identity provider, and complete
